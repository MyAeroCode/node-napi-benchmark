...
Napi::Object networkGet_n(const Napi::CallbackInfo& info)
{
    //
    // get argument.
    auto env = info.Env();
    ...
    //
    // reserve thread & data holder.
    std::vector<std::thread> getRequestThread;
    std::vector<std::string> getRequestThreadReturn;
    getRequestThread.reserve(parallel);
    getRequestThreadReturn.reserve(parallel);

    //
    // send request.
    curl_global_init(CURL_GLOBAL_ALL);
    for (auto i = 0; i < parallel; i++) {
        getRequestThreadReturn.emplace_back("");
        getRequestThread.emplace_back(std::thread(networkGet_impl, url, &getRequestThreadReturn[i]));
    }
    for (auto i = 0; i < parallel; i++) {
        getRequestThread[i].join();
    }
    curl_global_cleanup();
    ...
    for (auto i = 0; i < parallel; i++) {
        arr[i] = getRequestThreadReturn[i];
    }
...
{
    exports.Set("networkGet_n", Napi::Function::New(env, networkGet_n));
